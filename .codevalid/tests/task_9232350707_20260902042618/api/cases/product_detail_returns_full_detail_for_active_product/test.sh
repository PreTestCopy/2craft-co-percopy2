#!/usr/bin/env bash
# Test: product_detail_returns_full_detail_for_active_product
# Business: Marketplace Product Catalog Browsing and Individual Product Detail Retrieval for Buyers
set -euo pipefail
...
# ---------------------------------------------------------------------------
teardown() {
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM products WHERE id = '$PRODUCT_ID';" || true
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM seller_profiles WHERE id = '$SELLER_PROFILE_ID';" || true
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM users WHERE id = '$SELLER_ID';" || true
}
trap teardown EXIT
